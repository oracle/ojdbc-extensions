...
public class SerializerTestConverter {
  ...
  public void serialiZerTest() throws IOException {
      ConverterEntity entity = ConverterEntityInstance.getRandomEntity();
    ...
    try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
        try (OracleJsonGenerator generator = jsonFactory.createJsonBinaryGenerator(out)) {
          conv.serialize(generator, entity);
        }
        try(ByteArrayInputStream in = new ByteArrayInputStream(out.toByteArray())) {
          try (OracleJsonParser oParser = jsonFactory.createJsonBinaryParser(in)) {
            ConverterEntity deserEntity = (ConverterEntity) conv.deserialize(oParser, ConverterEntity.class);
            Assertions.assertEquals(entity, deserEntity);
          }
        }
