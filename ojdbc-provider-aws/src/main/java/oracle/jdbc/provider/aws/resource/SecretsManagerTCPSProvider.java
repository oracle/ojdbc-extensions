 ...
 * with an Oracle Autonomous Database. The file is retrieved from AWS Secrets Manager,
 ...
 * </p>
 */
public class SecretsManagerTCPSProvider
   ...
   */
  public SecretsManagerTCPSProvider() {
    super("secrets-manager-tls", PARAMETERS);
  }

  /**
   * Retrieves an SSLContext by loading a file from AWS Secrets Manager and
   * configuring it for secure TLS communication with Autonomous Database.
   * <p>
   * The file is stored in AWS Secrets Manager as a base64-encoded string.
   ...
   * including the secret name, password (if applicable), and
  ...
  public SSLContext getSSLContext(Map<Parameter, CharSequence> parameterValues) {
    try {
      ...
      byte[] fileBytes = Base64.getDecoder().decode(secret);
      ...
      throw new IllegalStateException("Failed to build SSLContext from AWS Secret", e);
    }
  }
}
