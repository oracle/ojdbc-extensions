 ...
 * with an Autonomous Database. The file is retrieved from GCP Secret Manager,
 * where it is stored either as a base64-encoded string or directly in binary
 * format (e.g., a wallet imported as a file). This provider supports
 ...
 * </p>
 */
public class GcpSecretManagerTCPSProvider
   ...
   */
  public GcpSecretManagerTCPSProvider() {
    super("secretmanager-tls", TCPS_PARAMETERS);
  }

  /**
   * Retrieves an SSLContext by loading a file from GCP Secret Manager and
   * configuring it for secure TLS communication with Autonomous Database.
   * <p>
   * The file is stored in GCP Secret Manager either as a base64-encoded string
   * or in its raw binary format.
   ...
   * including the secret version name, password (if applicable), and
