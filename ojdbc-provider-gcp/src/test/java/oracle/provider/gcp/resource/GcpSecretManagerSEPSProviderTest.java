...
import oracle.jdbc.provider.gcp.resource.GcpSecretManagerSEPSProvider;
...
public class GcpSecretManagerSEPSProviderTest {
  private static final UsernameProvider USERNAME_PROVIDER =
          findProvider(
            UsernameProvider.class, "ojdbc-provider-gcp-secretmanager-seps");

  private static final PasswordProvider PASSWORD_PROVIDER =
          findProvider(
            PasswordProvider.class, "ojdbc-provider-gcp-secretmanager-seps");


  /**
   * Verifies that
   * {@link GcpSecretManagerSEPSProvider#getParameters()} includes parameters
   * to configure a vault URL, secret name, wallet password, and connection
   * string index.
   */
  @Test
  ...
  public void testPKCS12PasswordWithConnectionStringIndex() {
    ...
    testParameters.put(
            "secretVersionName",
            TestProperties.getOrAbort(GcpTestProperty.GCP_PKCS12_SEPS_SECRET_VERSION_NAME));
    testParameters.put(
            "walletPassword",
            TestProperties.getOrAbort(GcpTestProperty.GCP_PKCS12_SEPS_WALLET_PASSWORD));
    testParameters.put(
            "connectionStringIndex",
            TestProperties.getOrAbort(GcpTestProperty.GCP_SEPS_CONNECTION_STRING_INDEX));
  ...
  public void testSSOPasswordWithConnectionStringIndex() {
    ...
    testParameters.put(
            "secretVersionName",
            TestProperties.getOrAbort(GcpTestProperty.GCP_SSO_SEPS_SECRET_VERSION_NAME));
    testParameters.put(
            "connectionStringIndex",
            TestProperties.getOrAbort(GcpTestProperty.GCP_SEPS_CONNECTION_STRING_INDEX));
  ...
  public void testCorruptedBase64SEPSWallet() {
    ...
    testParameters.put(
            "secretVersionName",
            TestProperties.getOrAbort(GcpTestProperty.GCP_CORRUPTED_SEPS_WALLET_SECRET_VERSION_NAME));
